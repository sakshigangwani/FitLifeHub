...
const Tall = ({navigation}) => {
    const { height, setHeight } = useUser(); 
    ...
    const [selectedTall, setSelectedTall] = useState(height || "182");

    useEffect(() => {
        if (progressBarWidth > 0) {
            Animated.timing(progressAnim, {
                toValue: progressBarWidth * 0.7,
                duration: 1000,
    ...
    return (
        <View style={styles.tallContainer}>
            ...
            <Animated.View style={[styles.greetingsContainer, { opacity: fadeAnim }]}>
                <Text style={styles.greetingsText}>How tall are you?</Text>
            ...
            <Animated.View style={[styles.formContainer, { opacity: fadeAnim }]}>
                <TextInput
                    style={styles.input}
                    placeholder={selectedTall}
                    keyboardType="default"
                    onChangeText={(text) => setSelectedTall(text)}
                    value={selectedTall}
                />
                <Text style={styles.yearsText}>Cm</Text>
            </Animated.View>
